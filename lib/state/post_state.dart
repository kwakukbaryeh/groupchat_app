...
class PostState extends AppStates {
  bool isBusy = false;
  Map<String, List<PostModel>?> groupChatPostMap = {}; // Use a map to store posts for each group chat
  Map<String, dabase.Query?> groupChatQueryMap = {}; // Use a map to store queries for each group chat
  ...
  void setFeedModel(PostModel model) {
    String groupChatId = model.groupChat!.key ?? 'default'; // Use 'default' as a fallback if groupChatId is null or empty
    ...
    try {
      ...
      if (groupChatQueryMap.isEmpty) {
        for (var groupChat in groupChats) {
          groupChatQueryMap[groupChat.key!] = kDatabase.child("posts").child(groupChat.key!);
          groupChatQueryMap[groupChat.key!]!.onChildAdded.listen((event) => onPostAdded(groupChat.key!, event));
        }
      }
          ...
          if (map != null) {
            ...
            map.forEach((groupChatId, value) {
              ...
              if (postMap != null) {
                ...
                postList.sort((x, y) => DateTime.parse(x.createdAt).compareTo(DateTime.parse(y.createdAt)));
